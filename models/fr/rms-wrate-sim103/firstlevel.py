...
def process_subject(inputpath, subjid, dtx_mat, outputpath):
    ...
    contrasts = {}
    con_names = ['rms', 'wrate', 'cwrate', 'simemb']
    ncon = len(dtx_mat[0].columns)
    con = np.zeros((len(con_names), ncon))
    con[:len(con_names),:len(con_names)] = np.eye(len(con_names))
    con[len(con_names)-1, len(con_names):] = 1
