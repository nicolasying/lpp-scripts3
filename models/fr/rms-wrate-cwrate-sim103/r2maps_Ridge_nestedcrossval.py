...
if __name__ == '__main__':
    ...
    alphas = [0] + list(np.logspace(1, 3, 100))
    # -3, 5, 20 gives best alpha in 2069, 5455.6 (3.31 - 3.7)
    # 3,4,20 left some best voxels out (per subject)
    # 1,3,100
    
    with open(output_dir + "run_newmask.log", "a+") as log_file:
        logcsvwriter = csv.writer(log_file)

    # logcsvwriter = csv.writer(open(output_dir + "run.log", "a+"))

        for subject in subjlist:

            loglabel = subject

            print('Main: computing for subject', subject)
            if op.isfile(output_dir + 'test_{}.nii.gz'.format(subject)):
